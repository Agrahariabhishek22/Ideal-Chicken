...
const DrumstickIcon = ({ size = 18, className }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" className={className}>
    <path fill="none" stroke="currentColor" strokeLinecap="round" strokeWidth="1.5" d="M18 8.034c-2.88 0-4.873-3.023-8-3.023c-4.418 0-8 3.61-8 8.063c0 1.433.171 3.138.868 4.535c.546 1.097 1.813 1.555 3.013 1.34C10.43 18.13 14 15.002 15 13.004m0 0c-1.769.106-4-.5-6-1.947m6 1.947c2.83-.17 7-2.796 7-5.978s-5-2.016-8-.56"/>
  </svg>
);
...
const PRODUCTS = [
  { id: 1, category: 'special', title: "Chicken Breast Boneless", desc: "Lorem ipsum dolor sit amet consectetur. Gravida elementum vel mollis quam.", image: "/pic1.png" },
  { id: 2, category: 'special', title: "Boneless Supreme", desc: "Lorem ipsum dolor sit amet consectetur. Gravida elementum vel mollis quam.", image: "/pic2.png" },
  { id: 3, category: 'special', title: "Chicken Lollypop", desc: "Lorem ipsum dolor sit amet consectetur. Gravida elementum vel mollis quam.", image: "/pic3.png" },
  { id: 4, category: 'special', title: "Mince", desc: "Lorem ipsum dolor sit amet consectetur. Gravida elementum vel mollis quam.", image: "/pic4.png" },
  { id: 5, category: 'special', title: "Hot Wings", desc: "Lorem ipsum dolor sit amet consectetur. Gravida elementum vel mollis quam.", image: "/pic3.png" },
  { id: 11, category: 'special', title: "Chicken Breast Boneless", desc: "Lorem ipsum dolor sit amet consectetur. Gravida elementum vel mollis quam.", image: "/pic1.png" },
  { id: 12, category: 'special', title: "Boneless Supreme", desc: "Lorem ipsum dolor sit amet consectetur. Gravida elementum vel mollis quam.", image: "/pic2.png" },
    ...
    <section className="w-full py-20 px-4 md:px-6 font-sans">
      <div className="max-w-[1520px] mx-auto">
        ...
        <div className="relative group px-0 md:px-8">
          ...
          <button 
            onClick={() => scroll('left')}
            className="hidden md:flex absolute -left-2 top-1/2 -translate-y-1/2 z-20 w-12 h-12 bg-[#B52A63] text-white rounded-full items-center justify-center shadow-lg hover:scale-110 transition-transform cursor-pointer"
          >
          ...
          <button 
            ...
            className="hidden md:flex absolute -right-2 top-1/2 -translate-y-1/2 z-20 w-12 h-12 bg-[#B52A63] text-white rounded-full items-center justify-center shadow-lg hover:scale-110 transition-transform cursor-pointer"
          >
          ...
          <div 
            ref={scrollRef}
            // CHANGED: Use grid with 2 columns on mobile, Flex on desktop
            className="grid grid-cols-2 gap-3 md:flex md:gap-6 md:overflow-x-auto scrollbar-hide pb-8 scroll-smooth"
          ...
          >
            {filteredProducts.map((product) => (
              ...
              <div 
                key={product.id} 
                // CHANGED: Width is full on mobile (to fit grid), 256px on desktop
                className="w-full min-w-0 md:min-w-[256px] md:w-[256px] md:shrink-0 group/card cursor-pointer"
              >
                {/* Image Card */}
                <div className="rounded-[1rem] md:rounded-[1.5rem] hover:shadow-xl transition-shadow duration-300">
                   {/* CHANGED: Height reduced on mobile to maintain aspect ratio */}
                   <div className="h-[160px] md:h-[220px] w-full rounded-[1rem] md:rounded-[1.5rem] overflow-hidden relative">
                     <img 
                       src={product.image} 
                       alt={product.title} 
                ...
                {/* Content */}
                <div className="mt-3 md:mt-5 px-1 md:px-2">
                  <h3 className="text-[#8B1E45] font-bold text-sm md:text-lg mb-1 md:mb-2 line-clamp-2 md:line-clamp-1 leading-tight">
                    {product.title}
                  </h3>
                  <p className="hidden md:block text-gray-500 text-xs leading-relaxed mb-4 line-clamp-2">
                    {product.desc}
                  </p>
                  
                  {/* Icon Only on Mobile, Icon + Desc on Desktop */}
                  <div className="md:mt-0 mt-2 w-7 h-7 md:w-9 md:h-9 rounded-full flex items-center justify-center transition-colors bg-gray-100 group-hover/card:bg-[#F9A8D4]">
                    <ArrowUpRight className="text-[#8B1E45] transition-transform duration-300 group-hover/card:rotate-45" size={16} />
                  </div>
                </div>
              </div>
            ))}
            ...
            {filteredProducts.length === 0 && (
              <div className="col-span-2 md:w-full text-center py-10 text-gray-400 italic">
